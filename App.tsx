...
interface StrategyConfig {
  ...
  targetAllocation: number; // Base/Fixed allocation
  minAllocation: number; // For Dynamic modes
  maxAllocation: number; // For Dynamic modes
  
  // Indicator Params
  indicatorPeriod: number; // RSI or ADX period
  adxThreshold: number; // Strength threshold for ADX
...
interface StepData {
  ...
  allocation: number; 
  targetAllocation: number; // Added to track dynamic target
  rsi?: number; // Debug/Viz
  adx?: number; // Debug/Viz
...
const generateJump = (config: MarketConfig): number[] => {
  ...
  for (let i = 1; i <= config.days; i++) {
    ...
    if (Math.random() < config.jumpIntensity * dt) {
      const jumpSize = generateGaussian(config.jumpMean, config.jumpStdDev);
...
const generatePricePath = (config: MarketConfig): number[] => {
  switch (config.type) {
    ...
    case 'GBM':
    default: return generateGBM(config);
  }
};

// --- Indicator Helpers Class ---
...
class TechnicalIndicators {
  ...
  private prices: number[] = []; // Keep track for SMA

  constructor(private period: number) {}

  // Returns RSI or null if not enough data
  updateRSI(price: number, prevPrice: number): number | null {
    ...
    // Simple Moving Average for RSI (Wilder's is better but SMA is common approx for sims)
  ...
  updateTrend(price: number, prevPrice: number): { adx: number, sma: number } | null {
    ...
    // Simplified ADX for Line Data (High=Low=Close approximation)
    // True Range approx = |Price - PrevPrice|
    // DM+ = (Price - PrevPrice) if > 0 else 0
    // DM- = (PrevPrice - Price) if > 0 else 0
    ...
    // For simplicity in a single pass loop, we return DX as ADX proxy (Smoothing DX requires another history buffer)
    // Or we can simple-smooth it: ADX = ((PrevADX * (n-1)) + DX) / n. 
    // Let's return DX for instant reaction in this simulation
    return { adx: dx, sma };
  }
}
...
const runSimulation = (
  ...
  // Initial Setup
  // Note: For dynamic strategies, we might start with base targetAllocation, 
  // then adjust as data comes in.
  let currentTargetAllocation = strategy.targetAllocation;
  if (strategy.allocationMode !== 'fixed') {
    currentTargetAllocation = 0.5; // Neutral start for dynamic
  }
  ...
  // HODL comparison always assumes fixed initial BUY of the base target allocation
  ...
  for (let t = 1; t < pricePath.length; t++) {
    ...
    const trendVal = indicators.updateTrend(price, prevPrice); // returns {adx, sma}

    // --- Calculate Dynamic Target ---
    if (strategy.allocationMode === 'rsi' && rsiVal !== null) {
        // RSI Mean Reversion Logic
        // RSI < 30 (Oversold) -> Target = Max (Buy)
        // RSI > 70 (Overbought) -> Target = Min (Sell)
        // RSI 50 -> Midpoint
        
        // Clamp RSI 30-70 to 0-1 range
        let rsiFactor = (rsiVal - 30) / 40; // 0 at RSI 30, 1 at RSI 70
        rsiFactor = Math.max(0, Math.min(1, rsiFactor)); 
        
        // Invert: Low RSI (0) should mean High Alloc. High RSI (1) should mean Low Alloc.
        ...
        // ADX Trend Following Logic
        // If ADX > threshold AND Price > SMA -> Strong Uptrend -> Max Alloc
        // If ADX > threshold AND Price < SMA -> Strong Downtrend -> Min Alloc
        // If ADX < threshold -> Choppy -> Neutral (Midpoint)
        ...
        if (adx > strategy.adxThreshold) {
           if (price > sma) {
             currentTargetAllocation = strategy.maxAllocation;
           } else {
             currentTargetAllocation = strategy.minAllocation;
           }
        } else {
    ...
    if (shouldRebalance) {
      ...
      const diff = targetAssetValue - currentAssetValue; // Positive = Buy, Negative = Sell
      
      // Minimal trade filter to avoid dust
      if (Math.abs(diff) > 1) {
        rebalanceCount++;
  ...
  const finalHodl = data[data.length - 1].hodlValue;
  const finalStrategy = data[data.length - 1].strategyValue;

  return {
    data,
    totalRebalances: rebalanceCount,
    totalFees,
    hodlReturn: (finalHodl - strategy.initialCapital) / strategy.initialCapital,
    strategyReturn: (finalStrategy - strategy.initialCapital) / strategy.initialCapital,
...
const NumberControl = ({ 
  ...
  const formatValue = (v: number) => {
    if (isPercentage) {
      return (Math.round(v * 10000) / 100).toString();
    }
    return v.toString();
  };
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!isNaN(parsed)) {
      const actualValue = isPercentage ? parsed / 100 : parsed;
  ...
  return (
    <div className={`mb-4 ${disabled ? 'opacity-50 pointer-events-none' : ''}`}>
      ...
      <div className="relative group">
        ...
        />
        {isPercentage && (
          <span className="absolute right-3 top-1/2 -translate-y-1/2 text-slate-500 text-xs font-medium pointer-events-none">
            %
          </span>
        )}
      </div>
...
const App = () => {
  ...
  const RebalanceDot = (props: any) => {
    ...
    if (payload.action === 'sell') {
      return <circle cx={cx} cy={cy} r={4} fill="#f43f5e" stroke="#fff" strokeWidth={1} />;
    }
    return null;
  };
        ...
        <div className="p-6 space-y-8 flex-1">
          ...
          <section>
            ...
            <div className="mb-4">
               ...
               <select 
                 value={market.type}
                 onChange={(e) => setMarket(p => ({ ...p, type: e.target.value as ModelType }))}
            ...
            <NumberControl 
              ...
              onChange={(v: number) => setMarket(p => ({ ...p, days: v }))} 
            />
            <NumberControl 
              ...
              isPercentage={true}
              onChange={(v: number) => setMarket(p => ({ ...p, volatility: v }))} 
            />

            {/* Dynamic Params */}
            {market.type === 'GBM' && (
              <NumberControl 
                label="Drift (Annual Trend)" 
                value={market.drift} 
                min={-0.5} max={0.5} step={0.01} 
                isPercentage={true}
                onChange={(v: number) => setMarket(p => ({ ...p, drift: v }))} 
              />
            )}
            ...
            {market.type === 'OU' && (
              <>
                <NumberControl 
                  ...
                  onChange={(v: number) => setMarket(p => ({ ...p, meanReversionSpeed: v }))} 
                />
                <NumberControl 
                  ...
                  prefix="$"
                  onChange={(v: number) => setMarket(p => ({ ...p, longTermMean: v }))} 
                />
              </>
            )}
            ...
            {market.type === 'JUMP' && (
              <>
                ...
                <NumberControl 
                  ...
                  onChange={(v: number) => setMarket(p => ({ ...p, jumpIntensity: v }))} 
                />
                <NumberControl 
                  label="Avg Jump Size" 
                  ...
                  min={-0.5} max={0.5} step={0.01} 
                  isPercentage={true}
                  onChange={(v: number) => setMarket(p => ({ ...p, jumpMean: v }))} 
                />
              </>
            )}
            
            <button 
              onClick={regenerateMarket}
              className="w-full mt-2 py-2 px-4 bg-slate-800 hover:bg-slate-700 border border-slate-600 rounded-lg text-xs font-medium text-indigo-300 transition-colors flex items-center justify-center gap-2"
            >
          ...
          <section>
            ...
            <NumberControl 
              ...
              prefix="$"
              onChange={(v: number) => setStrategy(p => ({ ...p, initialCapital: v }))} 
            />
            ...
            <div className="mb-4">
               ...
               <div className="flex flex-col gap-1">
                 <select 
                   ...
                   onChange={(e) => setStrategy(p => ({ ...p, allocationMode: e.target.value as AllocationMode }))}
                   className="w-full bg-slate-800 border border-slate-700 rounded-lg py-2 px-3 text-sm text-slate-200 focus:ring-2 focus:ring-indigo-500 outline-none"
                 >
            ...
            {strategy.allocationMode === 'fixed' ? (
              <NumberControl 
                ...
                value={strategy.targetAllocation} 
                min={0.0} max={1.0} step={0.01} 
                isPercentage={true}
                onChange={(v: number) => setStrategy(p => ({ ...p, targetAllocation: v }))} 
              />
            ) : (
              <>
                 <div className="grid grid-cols-2 gap-2">
                    <NumberControl 
                      label="Min Alloc" 
                      value={strategy.minAllocation} 
                      min={0.0} max={1.0} step={0.05} 
                      isPercentage={true}
                      onChange={(v: number) => setStrategy(p => ({ ...p, minAllocation: v }))} 
                    />
                    <NumberControl 
                      label="Max Alloc" 
                      ...
                      min={0.0} max={1.0} step={0.05} 
                      isPercentage={true}
                      onChange={(v: number) => setStrategy(p => ({ ...p, maxAllocation: v }))} 
                    />
                 </div>
                 <NumberControl 
                    label={strategy.allocationMode === 'rsi' ? "RSI Period" : "ADX Period"}
                    value={strategy.indicatorPeriod} 
                    min={2} max={50} step={1} 
                    onChange={(v: number) => setStrategy(p => ({ ...p, indicatorPeriod: v }))} 
                  />
                  {strategy.allocationMode === 'adx' && (
                     <NumberControl 
                        label="Trend Strength (ADX Threshold)" 
                        value={strategy.adxThreshold} 
                        min={10} max={50} step={1} 
                        onChange={(v: number) => setStrategy(p => ({ ...p, adxThreshold: v }))} 
                      />
                  )}
              </>
            )}
            ...
            {/* Rebalance Trigger Toggle */}
            <div className="mb-4">
               <label className="text-xs font-medium text-slate-300 mb-1.5 block">Rebalance Trigger</label>
               <div className="flex bg-slate-800 p-1 rounded-lg border border-slate-700">
                 <button
                   onClick={() => setStrategy(p => ({ ...p, rebalanceType: 'threshold' }))}
                   className={`flex-1 py-1.5 text-xs font-medium rounded transition-colors ${strategy.rebalanceType === 'threshold' ? 'bg-slate-600 text-white shadow-sm' : 'text-slate-400 hover:text-slate-200'}`}
                 >
                   Threshold
                 </button>
                 <button
                   onClick={() => setStrategy(p => ({ ...p, rebalanceType: 'time' }))}
                   className={`flex-1 py-1.5 text-xs font-medium rounded transition-colors ${strategy.rebalanceType === 'time' ? 'bg-slate-600 text-white shadow-sm' : 'text-slate-400 hover:text-slate-200'}`}
                 >
                   Time
                 </button>
               </div>
            ...
            {strategy.rebalanceType === 'threshold' ? (
              <NumberControl 
                ...
                isPercentage={true}
                onChange={(v: number) => setStrategy(p => ({ ...p, rebalanceThreshold: v }))} 
              />
            ) : (
               <NumberControl 
                ...
                onChange={(v: number) => setStrategy(p => ({ ...p, rebalanceFrequency: v }))} 
              />
            )}

            <NumberControl 
              label="Transaction Fee" 
              value={strategy.transactionFeeRate} 
              min={0.0} max={0.1} step={0.0001} 
              isPercentage={true}
              onChange={(v: number) => setStrategy(p => ({ ...p, transactionFeeRate: v }))}
            />
          </section>
        </div>
      </aside>
      ...
      <main className="flex-1 flex flex-col h-full overflow-hidden relative">
           ...
           {simResult && (
             <>
               <MetricCard 
                  ...
                  value={formatCurrency(simResult.data[simResult.data.length-1].strategyValue)} 
               ...
               <MetricCard 
                  ...
                  value={formatCurrency(simResult.data[simResult.data.length-1].hodlValue)} 
                  subValue={`${formatPercent(simResult.hodlReturn)} Return`}
                  icon={Maximize2}
               />
               <MetricCard 
                  label="Alpha (Strategy vs HODL)" 
                  value={formatCurrency(simResult.data[simResult.data.length-1].strategyValue - simResult.data[simResult.data.length-1].hodlValue)}
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-6">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            {/* Asset Allocation Heatmap/Chart */}
            <div className="bg-slate-900 border border-slate-800 rounded-xl p-4 shadow-sm">
               <div className="mb-4 flex justify-between items-center">
                ...
                <div className="text-xs text-indigo-400 flex flex-col items-end">
                  <span>
                     Mode: {strategy.allocationMode === 'fixed' ? 'Fixed' : (strategy.allocationMode === 'rsi' ? 'RSI Dynamic' : 'ADX Dynamic')}
                  </span>
